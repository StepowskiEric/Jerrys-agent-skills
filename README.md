...
npx jerry-skills install --agent codex --skill how-to-solve-it --skill ooda-loop
...
Each command copies every skill into a folder bundle with a `SKILL.md` file. Each bundle includes `name`, `description`, and `source: "jerry-skills"` frontmatter so skills are discoverable and identifiable in the skills picker. For example, `execution/how-to-solve-it-state-machine-skill.md` installs to `execution/how-to-solve-it-state-machine-skill/SKILL.md` under the target directory.
